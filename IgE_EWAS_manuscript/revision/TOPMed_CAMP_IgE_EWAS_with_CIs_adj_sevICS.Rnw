...
# include asthma severity measured by hosp visits and medication/ICS use

# Randomization variable for ICS use
pData.pheno.meth.camp$Steroid_binary=ifelse(pData.pheno.meth.camp$TG==1,1,0)
pData.pheno.meth.camp$Steroid_binary <- as.factor(pData.pheno.meth.camp$Steroid_binary)

# cumulative ED_hospitalization up to 4 year
pData.pheno.meth.camp$bin_cum_edhos_f48 <- as.factor(pData.pheno.meth.camp$bin_cum_edhos_f48)
...
formula <- c("~LOG10IGE_iuml_F48+age_f48+Gender+RACE+ETS+bin_cum_edhos_f48+Steroid_binary+MOMASM+Bcell+CD4T+CD8T+Mono+Neu+NK+SV1+SV2")

vars.of.interest <- c("LOG10IGE_iuml_F48")
varNames <- c("LOG10IGE_iuml_F48_ct_2SVs")
...
for (f in 1:length(formula)) {
  ...
  topHits <- limma::topTable(fit, confint=TRUE, coef=2, num=Inf, adjust.method="BH", 
  ...
  write.table(topHits,file=file.path(results.dir, 
                    paste0("CAMP_DMPs_topHits_all_IgE_withCIs_adj_sevICS_",gsub('\\.','',varNames[f]),
                           "_",timeStamp,".txt")),sep="\t",row.names=F,quote=F)
  
  write.table(tophits.fdr,file=file.path(results.dir, 
                    paste0("CAMP_DMPs_topHits_fdr_IgE_withCIs_adj_sevICS_",gsub('\\.','',varNames[f]),
                           "_",timeStamp,".txt")),sep="\t",row.names=F,quote=F)
  # volcano plot
...
lambda1 # 1.093738 for the selected model

png(file = file.path(plots.dir, "qqplot_log10Ige_CAMP_adj_sev_ICS.png"),
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{\Sexpr{plots.dir}/qqplot_log10Ige_CAMP_adj_sev_ICS}
  \end{center}
  \caption{Q-Q CAMP}
\end{figure}
...
formula <- c("~LOG10IGE_iuml_F48+age_f48+Gender+RACE+ETS+MOMASM+bin_cum_edhos_f48+Steroid_binary+Bcell+CD4T+CD8T+Mono+Neu+NK+SV1+SV2")
...
designCAMP = model.matrix(~LOG10IGE_iuml_F48+age_f48+Gender+RACE+ETS+MOMASM+bin_cum_edhos_f48+Steroid_binary+Bcell+CD4T+CD8T+Mono+Neu+NK+SV1+SV2, 
            data=pDat.reg)
...
save(myannotationCAMP, dmrcoutputCAMP, 
     file=file.path(results.dir,paste0("CAMP_regional_DMRs_hg19_results_logIgE_adj_sev_ICS_", 
                                       timeStamp,".RData")))

write.table(DMRsCAMP.sig, file.path(results.dir, 
      paste0("CAMP_DMRs_dmrcate_logIgE_hg19_fdr_adj_sev_ICS_", timeStamp, ".txt")),
