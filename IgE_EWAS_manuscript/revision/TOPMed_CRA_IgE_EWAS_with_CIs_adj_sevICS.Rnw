...
formula <- c("~log10Ige+age+Gender+Mothers_Asthma_Hx+smoking_early_life+Hospitalized_Asthma_Last_Yr+Inhaled_Steroids+Bcell+CD4T+CD8T+Mono+Neu+NK+SV1+SV2")

vars.of.interest <- c("log10Ige")
varNames <- c("log10Ige_ct2SVs")
...
remove <- c(list(which(is.na(pData.pheno.cra$age) | is.na(pData.pheno.cra$log10Ige) |  
                         is.na(pData.pheno.cra$Mothers_Asthma_Hx) | is.na(pData.pheno.cra$smoking_early_life) | 
                         is.na(pData.pheno.cra$Hospitalized_Asthma_Last_Yr))))

# include asthma severity measured by hosp visits in last year and medication/ICS use

length(remove[[1]])
sig.probes <- list()

pData.pheno.cra$Hospitalized_Asthma_Last_Yr <- as.factor(pData.pheno.cra$Hospitalized_Asthma_Last_Yr)
pData.pheno.cra$Inhaled_Steroids <- as.factor(pData.pheno.cra$Inhaled_Steroids)
...
for (f in 1:length(formula)) {
  ...
  topHits <- limma::topTable(fit, coef=2, confint=TRUE, num=Inf, adjust.method="BH", 
  ...
  write.table(topHits,file=file.path(results.dir, 
            paste0("CRA_DMPs_topHits_all_IgE_withCIs_adj_sevICS_",gsub('\\.','',
                                    varNames[f]),"_",timeStamp,".txt"))
              ,sep="\t",row.names=F,quote=F)
  write.table(tophits.fdr,file=file.path(results.dir, 
            paste0("CRA_DMPs_topHits_fdr_IgE_withCIs_adj_sevICS_",gsub('\\.','',
                                    varNames[f]),"_",timeStamp,".txt"))
              ,sep="\t",row.names=F,quote=F)
...
lambda1 # 1.380325 for the selected model

png(file = file.path(plots.dir, "qqplot_log10Ige_CRA_adj_sev_ICS.png"),
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{\Sexpr{plots.dir}/qqplot_log10Ige_CRA_adj_sev_ICS}
  \end{center}
  \caption{Q-Q CRA}
\end{figure}
...
designCRA = model.matrix(~log10Ige+age+Gender+Mothers_Asthma_Hx+smoking_early_life+Hospitalized_Asthma_Last_Yr+Inhaled_Steroids+Bcell+CD4T+CD8T+Mono+Neu+NK+SV1+SV2, data=pDat.reg)
...
save(myannotationCRA, dmrcoutputCRA, 
     file=file.path(results.dir,paste0("CRA_regional_DMRs_hg19_results_logIgE_adj_sev_ICS_", 
                                       timeStamp,".RData")))

write.table(DMRsCRA.sig, file.path(results.dir, 
      paste0("CRA_DMRs_dmrcate_logIgE_hg19_fdr_adj_sev_ICS_", timeStamp, ".txt")),
